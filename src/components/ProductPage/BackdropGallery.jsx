...
const BackdropGallery = ({ open, handleClose, currentPassedImage }) => {
  ...
  const removeActivatedClass = (parent) => {
    parent.childNodes.forEach((node) => {
      node.childNodes[0].classList.contains("activated") && node.childNodes[0].classList.remove("activated");
    });
  };
  ...
  return (
    ...
    >
      <section className="backdrop-content">
        <IconButton onClick={handleClose} sx={{ color: "#fff", bgcolor: "transparent", alignSelf: "flex-end" }}>
        ...
        <div className="image">
          ...
          <img src={backdropImage} alt="selected-product" style={{ cursor: "auto" }} />
        </div>
        <div className="thumbnails">
          {THUMBS.map((th, index) => {
            return (
              ...
              >
                <div className={`outlay ${index === currentPassedImageIndex && "activated"}`}></div>
